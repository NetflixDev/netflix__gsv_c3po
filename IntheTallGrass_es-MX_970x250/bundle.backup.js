(function (){
  var defaultData = {
    img: 'https://art-s.nflximg.net/50e0c/15722991e6d94c1af3d373cee1b67871d0450e0c.jpg',
    img2: 'https://art-s.nflximg.net/b560c/0af52356249725aaa518af7991431df5704b560c.png',
  ...
  var isImageLoaded1 = false
  var isImageLoaded2 = false
  ...
  function animateRibbon() {
    ...
    setTimeout(function () { l.transform = 'matrix(1, 0, 0, 1, 0, 0)' }, 0)
    setTimeout(function () { m.transform = 'translate(-50%, -50%) matrix(1, 0, 0, 1, 0, 0)' }, 0)
    setTimeout(function () { r.transform = 'matrix(1, 0, 0, 1, 0, 0)' }, 400)
    setTimeout(function () { l.transform = 'matrix(1, 0, 0, 1, 0, -250)' }, 1000)
    setTimeout(function () { m.transform = 'translate(-50%, 100%) matrix(1, 0, 0, 1, 0, 0)' }, 1200)
    setTimeout(function () { r.transform = 'matrix(1, 0, 0, 1, 0, -250)' }, 1400)
  }
  ...
  function animateLogo () {
    document.getElementById('logo').style.clip = 'rect(0px,110px,35px,0px)'
  }
  function showBug () {
  ...
  function initCta() {
    ...
    b.addEventListener('mouseover', function () { c.className = 'button hover isArrow' })
    b.addEventListener('mouseout', function () { c.className = 'button isArrow' })
  ...
  function injectData (data) {
    ...
    e = document.getElementById('title-img2')
    e.src = data[0].img2
    checkForLoaded(e, function () {
      isImageLoaded2 = true
      startAd()
    })
  ...
  function injectScaledText (container, className, text, left) {
    ...
    setTimeout(function () {
      function getPadding(el, s) { return parseInt(window.getComputedStyle(el, null).getPropertyValue('padding-' + s)) }
      var paddingX = getPadding(container, 'left') + getPadding(container, 'right')
      var paddingY = getPadding(container, 'top') + getPadding(container, 'bottom')
      ...
      if (left) x = 0;
      content.style.transform = 'scale(' + scale + ') translate('+x+', '+y+')'
    })
  }
  function loadFonts(locales, callback) {
      ...
      locales = locales.filter(function(value, index, self) { return self.indexOf(value) === index; });
  ...
  function startAd () {
    ...
    if (!isImageLoaded1) return
    if (!isImageLoaded2) return
  ...
  function verifyData (data) {
    if (typeof data.img !== 'string') throw new Error('Data Error: img = ' + data.img)
    if (typeof data.img2 !== 'string') throw new Error('Data Error: img2 = ' + data.img2)
    if (typeof data.text !== 'string') throw new Error('Data Error: text = ' + data.text)
    if (data.textLocale.length !== 2) throw new Error('Data Error: textLocale = ' + data.textLocale)
    if (typeof data.cta !== 'string') throw new Error('Data Error: cta = ' + data.cta)
    if (data.ctaLocale.length !== 2) throw new Error('Data Error: ctaLocale = ' + data.ctaLocale)
    if (typeof data.disclaimer !== 'string') throw new Error('Data Error: disclaimer = ' + data.disclaimer)
    if (data.disclaimerLocale.length !== 2) throw new Error('Data Error: disclaimerLocale = ' + data.disclaimerLocale)
    if (typeof data.url !== 'string') throw new Error('Data Error: url = ' + data.url)
    if (typeof data.templateBlack !== 'boolean') throw new Error('Data Error: templateBlack = ' + data.templateBlack)
    if (typeof data.ratingBug !== 'string') throw new Error('Data Error: ratingBug = ' + data.ratingBug)
  }
    ...
    Monet.logEvent("MONET_ASSETS_LOADED");
      try {
        ...
        monetAssets.assetGroups.forEach(function(d, i) {
          data[i] = {
            img: d['text.Netflix_Title_Type'].text == "Original" ? 
                  d['image.Single_Image_970x250_StoryArt_C20'].url : 
                  d['image.Single_Image_970x250_SDP_C20'].url,
            img2: d['image.Title_Treatment_C20'].url,
  ...
  if (window.Enabler) {
    initCta()
    var errorTimeout = setTimeout(function () { handleError(new Error('timeout')) }, 10000)
