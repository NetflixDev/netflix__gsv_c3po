!(function() {
  ...
  function parseDistanceAttr(distAttr) {
    if (typeof distAttr === "number") {
      return distAttr + "px";
    }

    var parsedNum = parseFloat(distAttr);
    // is number string --> convert to px
    if (!isNaN(parsedNum) && /^(\d|\.)+$/.test(distAttr)) {
      return parsedNum + "px";
    }
    return distAttr;
  }
  ...
  function style(isRTL) {
    ...
    Utils.createStyle.call(
      ...
      '.button .content .copy',
        'will-change: transform;transform-origin: 0 0;letter-spacing:' + this.data.letterSpacing +
          ';transition: color ' +
          TRANSITION_EASING +
          ';color:' +
          this.data.color[1],
  ...
  var CtaComponent = function() {
    ...
    self.button.className = "button";
    self.fill = document.createElement("div");
    self.fill.className = "fill";
    self.content = document.createElement("div");
    self.content.className = "content";
    self.copy = document.createElement("div");
    self.copy.className = "copy";
    self.arrow = document.createElement("div");
    self.arrow.className = "arrow";
    self.border = document.createElement("div");
    self.border.className = "border";
    return self;
  };
  ...
  CtaComponent.prototype = Object.create(HTMLElement.prototype, {
    ...
    populateData: {
      value: function() {
        this.data = {};
        this.data.color = [
          this.getAttribute("color-1") || "#e50914",
          this.getAttribute("color-2") || "#ffffff"
        ];
        this.data.size = parseInt(this.getAttribute("font-size"), 10) || 12;
        this.data.minSize =
          parseInt(this.getAttribute("min-font-size"), 10) || 8;
        this.data.font =
          (this.getAttribute("font") || "Netflix Sans") + ", Arial, sans-serif";
        this.data.text = this.getAttribute("text");
        this.data.multiLineHeight = this.getAttribute("multi-line-height");
        this.data.horizPad =
          parseDistanceAttr(this.getAttribute("horizontal-pad")) || "5%";
        this.data.vertPad =
          parseDistanceAttr(this.getAttribute("vertical-pad")) || "3%";
        this.data.letterSpacing =
          parseDistanceAttr(this.getAttribute("letter-spacing")) || "0px";
    ...
    connectedCallback: {
      value: function() {
        ...
        if (bgImg) {
          ...
          this.bgImgContainer.setAttribute(
            "style",
            "position: absolute; top:0;left:0;"
          );
        }
        ...
        this.button.addEventListener(
          ...
          function(event) {
            ...
            Monet.trackEvent("CLICK", {
              src: event.target.id || COMPONENT_NAME,
              coords: {
                x: event.clientX,
                y: event.clientY
              }
        ...
        var FontsComponent = (this.fontsComponent =
          this.fontsComponent || document.querySelector("netflix-fonts"));
        ...
        if (MonetComponent) {
          ...
          ensureFontsReady()
            .then(function() {
              return MonetComponent.ensureInitialized();
            })
            .then(
              function() {
                ...
                try {
                  cta = Monet.getFieldValue(fieldName);
                  var rawLocale = Monet.getLocale();
                  var locale = rawLocale && rawLocale.substr(0, 2);
                  ...
                  Monet.trackEvent("MONET_DATA_ERROR", {
                    details:
                      "Netflix CTA Component error; Could not find data in rootAssets: " +
                      fieldName,
                    stack: e
                  });
                }
    ...
    _getLocaleFont: {
      value: function() {
        ...
        if (!this.localeFont) {
          ...
          var localeFonts =
            fontLoads &&
            (fontLoads[this.locale] && fontLoads[this.locale].length
              ? fontLoads[this.locale]
              : fontLoads["en"]);
          this.localeFont =
            localeFonts &&
    ...
    _measureInlineSpaces: {
      value: function() {
        ...
        if (measurer) {
          ...
          if (
            localeFont.family === "Netflix Sans" ||
            localeFont.family === "Neue Helvetica Arab"
          ) {
    ...
    _measureBlockSpaces: {
      value: function() {
        ...
        return {
          ...
          bottom: Math.abs(
            blockRenderedSpanRect.bottom - inlineRenderedSpanRect.bottom
          )
        };
      }
    },
    ...
    _preventMeasuringAlignFix: {
      value: function() {
        ...
        return (
          (this.locale === "th" && this._renderedFontSize >= 10) ||
          isNetflixFont
        );
      }
    },
    ...
    _applyContentFixes: {
      value: function() {
        ...
        var copyYFixOnMultiLine = this._hasTruthyAttribute(
          "y-fix-on-multiLine"
        );
        ...
        var yFixToUse = !isNaN(parsedCopyYFix)
          ? parsedCopyYFix
          : this._measureCenterFix();

        if (parsedCopyXFix || yFixToUse) {
          TweenMax.set(this.copy, {
            x: !isNaN(parsedCopyXFix) ? parsedCopyXFix : 0,
            y:
              !isNaN(yFixToUse) && (!this._multiLine || copyYFixOnMultiLine)
                ? yFixToUse
                : 0
          });
        }
        ...
        if (this.hasArrow) {
          ...
          if (parsedArrowXFix || parsedArrowYFix) {
            TweenMax.set(this.arrow, {
              ...
              y:
                arrowYFix && (!this._multiLine || arrowYFixOnMultiLine)
                  ? parsedArrowYFix
                  : 0
            });
          }
    ...
    _clampContents: {
      value: function(spanWidth, maxContentWidth, arrowHeight) {
        ...
        // if CTA has an arrow
        if (this.hasArrow) {
          var arrowSpaceRatio = this._multiLine
            ? this.data.multilineCopyArrowSpaceRatio
            : this.data.copyArrowSpaceRatio;
    ...
    resize: {
      value: function(w, h, options) {
        ...
        var height =
          h || (this.getAttribute("height") || (this.offsetHeight || 28));
        ...
        var copyToContentRatio = this.hasArrow
          ? this.data.arrowCopyContentRatio
          : 1;
        ...
        this.content.style.height = this.copy.style.height =
          contentHeight + "px";

        this.copy.style.whiteSpace = options.tryingMultiLine
          ? "normal"
          : "nowrap";
        ...
        var fontSize = (this._renderedFontSize = parseInt(
        ...
        if (copyExcessWidth > 0 || copyExcessHeight > 0) {
          ...
          if (!options.tryingStretch) {
            ...
            if (horizPadIsPercent) {
              ...
              potentialNewWidth =
                ((copyContainerWidth + copyExcessWidth) *
                  (1 + 2 * horizPadPercent)) /
                  copyToContentRatio +
                1;
            } else {
              ...
              potentialNewWidth =
                (copyContainerWidth + copyExcessWidth) / copyToContentRatio +
                2 * horizPadVal +
                1;
            }
            ...
            var newWidth =
              copyExcessHeight > 0
                ? maxWidth
                : Math.min(maxWidth, potentialNewWidth);
    ...
    mouseover: {
      value: function() {
        ...
        this.arrow
          .querySelector("svg")
          .line.setAttribute("stroke", this.data.color[0]);
      }
    },
    ...
    mouseout: {
      value: function() {
        ...
        this.arrow
          .querySelector("svg")
          .line.setAttribute("stroke", this.data.color[1]);
      }
    },
